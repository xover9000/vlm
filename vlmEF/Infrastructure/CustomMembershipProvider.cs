...
namespace vlmEF.Infrastructure
{
    public class CustomMembershipProvider : MembershipProvider
    {
        ...
        public override MembershipUser CreateUser(string username, string password, string email, string passwordQuestion, string passwordAnswer, bool isApproved, object providerUserKey, out MembershipCreateStatus status)
        {
            ...
            if (user == null)
            {
                var userObj = new User { UserName = username, Password = GetMd5Hash(password), EmailAddress = email };

                using (var usersContext = new UsersContext())
                {
                    usersContext.AddToUsers(userObj);
        ...
        public override bool ValidateUser(string username, string password)
        {
            ...
            using (var usersContext = new UsersContext())
            {
                var requiredUser = usersContext.Users.FirstOrDefault(x => x.UserName == username && x.Password == md5Hash);
